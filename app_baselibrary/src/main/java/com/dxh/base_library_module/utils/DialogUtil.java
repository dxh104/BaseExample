...
public class DialogUtil {
    ...
    public static Dialog createBaseDialog(final Context context) {
        ...
        final Dialog dialog = new Dialog(context, R.style.BaseBlackDialog);//设置透明样式
        dialog.setContentView(view);//对话框填充布局
        dialog.getWindow().setGravity(Gravity.BOTTOM);//窗口靠底
        dialog.getWindow().setWindowAnimations(R.style.BaseDialogBottomAnimation);//对话框底部弹出动画样式
